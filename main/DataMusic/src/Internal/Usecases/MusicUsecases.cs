...
)
{
    public async Task SaveMusic(string name)
    {
        ...
        var response = await service.RequestExternalApi<ItunesResponse<MapMusicJson>>(url);
        ...
        if (album is null)
        {
            var newAlbum = new Album(
                originalId: result.CollectionId,
                name: result.CollectionName,
                fkArtists: artistId!
            );
            albumId = await repoAlbum.Save(newAlbum);
        }
    ...
    public async Task<List<Music>> GetMusic(QueryMusic info)
    {
