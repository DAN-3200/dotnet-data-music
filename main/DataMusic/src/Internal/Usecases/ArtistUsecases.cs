...
)
{
    public async Task SaveArtist(string name)
    {
        ...
        var url = $"https://itunes.apple.com/search?term={name}&media=music&entity=musicArtist&limit=1";
        var response = await service.RequestExternalApi<ItunesResponse<MapArtistJson>>(url);
    ...
    public async Task<List<Artist>> GetArtist(QueryArtist info)
    {
